...
class Calculator {
	...
	/**
	 * Generates a list of focusable elements and removes the two anchor spans.
	 */
	...
	findElementAfter = (element, search) => {
	...
	findElementBefore = (element, search) => {
		let all = Array.from(document.querySelectorAll("*"));
		all = all.reverse();
		let index = all.indexOf(element);
		let after = all.slice(index + 1);
	...
	convertColorant = (colorant, check) => {
	...
	subtractColorants = (colorant1, colorant2) => {
