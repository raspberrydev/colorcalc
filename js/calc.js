...
class Calculator {
  constructor(setup) {
    _defineProperty(this, "id", () => "_" + Math.random().toString(36).substr(2, 4));

    _defineProperty(this, "generateFocusableElements", () => {
      this.focusableElements = Array.from(document.querySelectorAll(`a[href], button, input, textarea, select, details, [tabindex]:not([tabindex="-1"])`)).filter(el => !el.hasAttribute("disabled") && !el.getAttribute("aria-hidden"));
      this.focusableElements.shift(); // Remove first (span)

      this.focusableElements.pop(); // Remove last (span)
    });

    _defineProperty(this, "setupLookup", () => {
      let headers = ["ID", "Colorant Name"];
    ...
    _defineProperty(this, "divide", (x, y) => {
      let result = x / y;
      return {
    ...
    _defineProperty(this, "chunkSplit", (arr, bulkSize = 20) => {
      let bulks = [];
    ...
    _defineProperty(this, "find", (direction, array, item) => {
      let items = array.filter(element => !element.readOnly);
      let index = items.indexOf(item);

      if (direction > 0) {
        return index + 1 == items.length ? item : items[index + 1];
      } else if (direction < 0) {
        return index == 0 ? item : items[index - 1];
      }
    });

    _defineProperty(this, "findElementAfter", (element, search) => {
    ...
    _defineProperty(this, "findElementBefore", (element, search) => {
      let all = Array.from(document.querySelectorAll("*"));
      all = all.reverse();
      let index = all.indexOf(element);
      let after = all.slice(index + 1);
    ...
    _defineProperty(this, "convertColorant", (colorant, check) => {
      if (typeof colorant === "number") {
        let oz = ~~colorant,
            dec1 = this.divide(this.divide(colorant - oz, 1 / 128).result, 4),
            c32 = dec1.whole,
            dec2 = this.divide(dec1.remainder, 1 / 2),
            c64 = dec2.whole,
            dec3 = this.divide(dec2.remainder, 1 / 2),
            c128 = dec3.whole;
    ...
    _defineProperty(this, "subtractColorants", (colorant1, colorant2) => {
    ...
    _defineProperty(this, "isColorant", element => {
      let id = element.value;
      let table = this.findElementBefore(element, x => x.classList.contains("color"));

      let isUnique = (table, id) => {
        // Prevent duplicate colorants
    ...
    _defineProperty(this, "createInput", (rid, size, cid) => {
      let div = document.createElement("div");
    ...
    _defineProperty(this, "createHeader", () => {
      let div = document.createElement("div");
      div.className = "header";
    ...
    _defineProperty(this, "createRow", cid => {
      let columns = [];
      let rid = this.id();
    ...
    _defineProperty(this, "createResultRow", colorant => {
      let columns = [];
    ...
    _defineProperty(this, "createResultInput", (size, value) => {
      let div = document.createElement("div");
    ...
    _defineProperty(this, "createResultTable", colorants => {
    ...
    _defineProperty(this, "createColorTable", () => {
      let table = document.createElement("div");
    ...
    _defineProperty(this, "decodeColorantTable", element => {
    ...
    _defineProperty(this, "getColorant", (color, id) => {
    ...
    _defineProperty(this, "calculate", () => {
      ...
      for (let color of similar) {
        //console.log(color)
        similarColorants.push([this.getColorant(color1, color[0]), this.getColorant(color2, color[0])]);
      }
      ...
      document.getElementById("final-table").appendChild(this.createResultTable(totalAdditions));
      ...
      if (different1.length < 1) {
        ...
        document.getElementById("missing-table").appendChild(this.createResultTable(different1));
      }
    ...
    _defineProperty(this, "bindInputs", refresh => {
      /**
      ...
      Array.from(document.querySelectorAll("input")).forEach(input => {
        ...
        input.addEventListener("input", input._input = event => {
          if ("numeric" in event.target.dataset) {
            // If input has numeric tag, force its value to match
        ...
        input.addEventListener("keydown", input._keydown = event => {
          ...
          if (event.key == "ArrowDown" && event.target.tagName.toLowerCase() == "input" && !this.colorantLookupOpen) {
            let next = this.findElementAfter(event.target, x => x.className == event.target.className);
            next ? next.focus() : "";
            return;
          }

          if (event.key == "ArrowUp" && event.target.tagName.toLowerCase() == "input" && !this.colorantLookupOpen) {
            let before = this.findElementBefore(event.target, x => x.className == event.target.className);
            before ? before.focus() : "";
            return;
          }

          if (event.key == "ArrowRight" && event.target.tagName.toLowerCase() == "input" && !this.colorantLookupOpen) {
            let next = this.findElementAfter(event.target, x => x.dataset.rid == event.target.dataset.rid && x.readOnly == false);
            next ? next.focus() : "";
            return;
          }

          if (event.key == "ArrowLeft" && event.target.tagName.toLowerCase() == "input" && !this.colorantLookupOpen) {
            let before = this.findElementBefore(event.target, x => x.dataset.rid == event.target.dataset.rid && x.readOnly == false);
            before ? before.focus() : "";
            return;
          }

          if (event.key == "Enter" && !this.colorantLookupOpen) {
            // Make enter button act like tab to move to next input
          ...
          if (event.key == "Backspace") {
            // Make backspace button act like shift+tab to move to previous input
          ...
          let colorantIDElement = document.getElementById(`c${event.target.dataset.rid}-id`); // Check the validity of the colorant id before allowing modification of the other value inputs
        ...
        input.addEventListener("blur", input._blur = event => {
          ...
          if ("colorant" in event.target.dataset) {
            if (!this.isColorant(event.target)) {
              // If the user attempts to blur a colorant entry input with an invalid id, we retain focus and add an error state
              if (event.target.value.length == 0) {
      ...
      document.addEventListener("keydown", document._keydown = event => {
        ...
        if (event.key == "Enter" && this.colorantLookupOpen) {
          ...
          this.findElementAfter(this.colorantLookupElement, x => x.classList.contains("coz")).focus();
          ...
          this.colorantLookupOpen = false; //document.activeElement.value = 
        }
    ...
    setup.elements.original.appendChild(this.createColorTable(0));
    this.originalColor = setup.elements.original.firstElementChild;
    setup.elements.new.appendChild(this.createColorTable(config.rowCount) // using config.rowCount to offset ids
    );
